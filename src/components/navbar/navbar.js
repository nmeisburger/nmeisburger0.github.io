...
class Navbar extends React.Component {
    ...
    render() {
        ...
        let barOffsetY = -160 + 40 * this.state.currentIdx;
        let h = this.state.expanded ? (200) : (0)

        let toggleSyle = { transition: 'transform 0.2s ease-in-out', marginLeft: '-85%' }
        ...
        return (
            <div className='navbar'>
                    ...
                    <div className='navbar-links-container'>
                        {
                            links.map(link => (
                                <Scroll.Link to={link.to} smooth={true} duration={400} key={`${link.name}`}>
                                    <div onClick={() => this.updateIdx(links.indexOf(link))}
                                        className={links.indexOf(link) === this.state.currentIdx ? ('navbar-active') : ('navbar-inactive')}>
                                        {link.name}
                                    </div>
                                </Scroll.Link>
                            ))
                ...
                <div className='navbar-narrow'>
                    <img src='https://icon.now.sh/burger/87ADC6/40' alt="" onClick={this.expand} style={toggleSyle} />
                        ...
                        <div>
                            {
                                links.map(link => (
                                    <div onClick={() => this.updateIdx(links.indexOf(link))} onMouseEnter={() => this.updateIdx(links.indexOf(link))}
                                        className={links.indexOf(link) === this.state.currentIdx ? ('navbar-active-narrow') : ('navbar-inactive-narrow')} key={`${link.name}`}>
                                        {link.name}
                                    </div>
                                ))
                            }
